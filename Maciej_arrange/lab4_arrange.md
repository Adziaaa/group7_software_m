...
The dominant smell is **Duplicated Code** (Kerievsky, *Refactoring to Patterns*, Chapter 4), with a strong element of **Alternative Classes with Different Interfaces**.
...
abstract class AbstractArrangeAction extends AbstractSelectedAction {
    ...
    protected abstract String getPresentationId();

    @Override
    public final void actionPerformed(ActionEvent e) {            // <-- template method
        ...
        fireUndoableEditHappened(new AbstractUndoableEdit() {
            public String getPresentationName() {
                return ResourceBundleUtil.getBundle("org.jhotdraw.draw.Labels")
                        .getTextProperty(getPresentationId());
            }
            public void redo() { super.redo(); arrange(view, figures); }
            public void undo() { super.undo(); reverseArrange(view, figures); }
        });
    }
}
```

Each subclass shrinks to the few lines that actually differ.
...
3. **Form Template Method** — pull the now-identical `actionPerformed` skeleton up into the superclass, and replace the differing steps with calls to abstract primitive operations (`arrange`, `reverseArrange`, `getPresentationId`).
